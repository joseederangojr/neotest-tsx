...
-- Synchronously runs `tsx` in `file_path` directory skipping all tests and returns `tsx` output.
-- Output have all of the test names inside it. It skips all tests by adding
-- extra `--testPathPattern` parameter to tsx command with placeholder string that should never exist.
-- @param file_path string - path to file to search for tests
-- @return table - parsed tsx test results
local function run_tsx_test_discovery(file_path)
  local binary = tsx_util.getTsxCommand(file_path)
  ...
  local tsx_json_string = result[2].stdout

  if not tsx_json_string or #tsx_json_string == 0 then
    return nil
  end

  return vim.json.decode(tsx_json_string, { luanil = { object = true } })
end

-- Searches through whole `tsx` command output and returns array of all tests at given `position`.
-- @param tsx_output table
-- @param position number[]
-- @return { keyid: string, name: string }[]
local function get_tests_ids_at_position(tsx_output, position)
  local test_ids_at_position = {}
  for _, testResult in pairs(tsx_output.testResults) do
    ...
    for _, assertionResult in pairs(testResult.assertionResults) do
      ...
      if position[1] <= location.line - 1 and position[3] >= location.line - 1 then
        local keyid = tsx_util.get_test_full_id_from_test_result(testFile, assertionResult)
...
-- First runs `tsx` in `file_path` to get all of the tests in the file. Then it takes all of
...
function M.enrich_positions_with_parameterized_tests(
  ...
  local tsx_test_discovery_output = run_tsx_test_discovery(file_path)

  if tsx_test_discovery_output == nil then
    return
  end
  ...
  for _, value in pairs(parsed_parameterized_tests_positions) do
    ...
    local parameterized_test_results_for_position =
      get_tests_ids_at_position(tsx_test_discovery_output, data.range)
...
-- Replaces all of the tsx parameters (named and unnamed) with `.*` regex pattern.
...
function M.replaceTestParametersWithRegex(test_name)
  ...
  for _, parameter in ipairs(TSX_PARAMETER_TYPES) do
